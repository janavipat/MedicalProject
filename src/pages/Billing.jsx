...
import {
  ...
  Loader2, RefreshCw, AlertCircle, Plus, X, Trash2, User, Info,
  QrCode, Share2, MessageCircle, Copy, CheckCheck, Wifi,
} from 'lucide-react';
...
// ── UPI payment helpers ───────────────────────────────────────────────────────
const CLINIC_UPI   = 'clinic@upi';   // ← replace with your actual UPI ID
...
function PaymentQRModal({ bill, onClose, onPaid, authFetch }) {
  ...
  // Poll backend every 5 s for payment confirmation
  useEffect(() => {
    if (paid) return;
          ...
          {paid ? (
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button
                  onClick={copyLink}
                  style={{ flex: 1, padding: '10px', borderRadius: '10px', border: '1.5px solid #e2e8f0', background: copied ? '#f0fdf4' : 'white', color: copied ? '#16a34a' : '#374151', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
                >
                ...
                <button
                  ...
                  style={{ flex: 1, padding: '10px', borderRadius: '10px', border: 'none', background: '#25D366', color: 'white', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
                >
                  <MessageCircle size={15} /> Send WhatsApp
                </button>
              </div>
              <div style={{ marginTop: '12px', fontSize: '0.72rem', color: '#9ca3af' }}>
                UPI ID: {CLINIC_UPI} · Status auto-updates every 5s
              </div>
            </>
          )}
        </div>
                  ...
                  <tr key={bill._id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        ...
                        <button className="btn btn-outline" style={{ padding: '5px 8px', fontSize: '0.8rem' }}><FileText size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
